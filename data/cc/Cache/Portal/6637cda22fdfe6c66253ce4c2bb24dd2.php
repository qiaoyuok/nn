...
.myButton .scoreList .scoreItem{width: 6.7vh;height:7.2vh;background:url("/app/img/round.png");background-size:106.8vh 12.8vh;float: left;margin-left: 2.8vh;}
...
.myButton .doubleButton .isOver .button{width: 6.7vh;height:7.2vh;background:url("/app/img/round.png");background-size:106.8vh 12.8vh;}
...
function userdjs(data) {
    clearTimeout(ji);
    var now = data.time - Math.ceil(new Date / 1e3) - (0 - timewc) - 1;
    if (0 < now) {
        ...
        ji = setTimeout("userdjs(" + JSON.stringify(data) + ")", 1e3)
    } else clearuserdjs()
}
...
function allfapai(data) {
    ...
    for (var i = 0; i < data.user.length; i++) {
        ...
        html = "";
        for (indexuser <= 0 && (indexuser -= -9), j = 1; j < 4; j++) html = data.card ? 6 <= indexuser ? html + '<div class="card cardall card' + indexuser + j + " card" + data.card[user.id][j - 1].card + " zhajinhua-index" + (4 - j) + '"  style="display:none" ></div>': html + '<div class="card cardall card' + indexuser + j + " card" + data.card[user.id][j - 1].card + " zhajinhua-index" + j + '"  style="display:none" ></div>': html + '<div class="card card' + indexuser + j + "  zhajinhua-index" + j + '"  style="display:none" ></div>';
        $("#userfp").append(html)
    }
    setTimeout(function() {
...
function havelose(data) {
    var indexuser = data.index - index - -1;
    if (indexuser <= 0 && (indexuser -= -9), 1 == indexuser) {
        hidebook(),
        $("#member .member1 .quitBack").show(),
        $(".myCards .isLose").show(),
        $(".myQuitBack").show();
        for (var i = 0; i < 3; i++) $(".cardDeal .card1" + (i - -1)).hide(),
        $(".myCards .card" + i).show()
    } else {
...
function scoresArea(data) {
    ...
    jf = $("#member .member" + indexuser + " .score").html();
    $("#member .member" + indexuser + " .score").html(jf - data.jf),
    $(".totalScore .scores").html(total - (0 - data.jf)),
    $(".scoresArea").append("<div class='coin  coin" + indexuser + " coinType" + data.jf + " style='top:" + 112 * Math.random() + "px;left:" + 112 * Math.random() + "px;' ></div>"),
...
function recm(data) {
    for (var total = 0,
    i = 0; i < data.cm.length; i++) total -= 0 - data.cm[i],
    $(".scoresArea").append("<div class='coin  coinType" + data.cm[i] + "' style='top:" + 112 * Math.random() + "px;left:" + 112 * Math.random() + "px;' ></div>");
...
function userUpdate(data) {
  console.log(data);
  if(data.is_start == 0){
      $("#watch-list .joimwatch").show();
  } else {
      $("#watch-list .joimwatch").hide();
  }
...
<textarea id="room_str" value=""><?php echo ($titlexx); ?>区: 9人诈金花 <?php echo ($room["roomid"]); ?>房间&#10规则:<?php if($room['df']): echo ($room['df']); ?>底；<?php endif; if($room['kp']): ?>看牌：<?php echo ($room['kp']); ?>；<?php endif; if($room['bp']): ?>比牌：<?php echo ($room['bp']); ?>；<?php endif; if($room['sx']): ?>上限：<?php echo ($room['sx']); endif; ?>&#10房间链接:&#10<?php echo ($room["dwz"]); ?></textarea>
...
<img src="/app/img/footer_bull.png" usemap="#planetmap" class="bottom" style="pointer-events: none;"/> 
  ...
  <img src="/app/img/icon_sound61.png" class="bottomGameHistory" style="z-index: 110;position: fixed;bottom:5;left: 60px;width: 30px;height: 30px;z-index:90;"onclick="opnemm('fangjian_yinyue','vaudioRoom')" /> 
