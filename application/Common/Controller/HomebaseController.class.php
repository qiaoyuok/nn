...
if (strpos($_SERVER['HTTP_USER_AGENT'], 'XLMessenger') == true) {
  //闲聊浏览器打开 执行登陆判断
...
}elseif(strpos($_SERVER['HTTP_USER_AGENT'], 'XLMessenger') ==false && strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') ==false && strpos($_SERVER['HTTP_USER_AGENT'], 'QQ/')==false){
  if(!$_SESSION['uid'] && !$_GET['code']){
   $baseurl='http://'.$_SERVER['HTTP_HOST'].'/BrowserLogin/keyLogin.html?'.$_SERVER['QUERY_STRING'];header("Location: ".$baseurl);exit;
  }
