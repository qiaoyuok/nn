...
if ($Room[$id]['disable_pk'] == 1) {
    $biuser['user'] = array();
}
if ($Room[$id]['total'] < $Room[$id]['bp']) {
    $biuser['user'] = array();
}

if ($Room[$id]['count'] < $Room[$id]['pk_round']*count($Room[$id]['list'])) {
    $biuser['user'] = array();
}
...
foreach ($Room[$id]['user'] as $connection3) {
    ...
    if ($connection3->user['index'] == $msg['index']) {
        ...
        if ($connection3->user['tpzt'] != 1 && $Room[$id]['total'] < $Room[$id]['kp'] && $Room[$id]['disable_pk'] == 1) {
            act('hidebook', '', $connection3);
			return false;
        } 
        if ($Room[$id]['total'] < $Room[$id]['kp'] ) {
            act('hidebook', '', $connection3);
			return false;
        } 
              if (      $Room[$id]['disable_pk'] == 1) {
