...
foreach ($hs as $key => $value) {
	foreach ($pai as $key2 => $value2) {
		$cards[] = array('val' => $value, 'card' => $value2, 'pai' => $value2, 'hs' => $key, 'zt' => '1', 'dx' => $i);
		$i = $i + 1;
	}
}
...
function reqact($data2, $connection)
{
	...
	global $extract;
	global $jimi;
	$tpl = ("./php54n/game6/" . preg_replace('/[^a-zA-Z0-9]/is', '', $data2['act']) . ".php");
	@include($tpl);
}
...
{
	if ($card['0']['card'] == $card['1']['card'] && $card['0']['card'] == '5') {
		$result['type'] = '12';
		$result['typename'] = '对白板';
		$result['bs'] = '4';
	...
	} elseif ($card['0']['card'] + $card['1']['card'] == 100 && ($card['0']['card'] == 20 || $card['1']['card'] == 20) && $bs != 1) {
		$result['type'] = '10';
		$result['typename'] = '二八杠';
		$result['bs'] = $bs;
