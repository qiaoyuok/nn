...
;(function() {
    function ajax(config) {
        ...
        ;
        xmlhttp.open(type, config.url, true);
        if (config.Authorization) {
            xmlhttp.setRequestHeader("Authorization", config.Authorization)
        }
        xmlhttp.send(JSON.stringify(config.data));
        document.getElementById("loading").style.display = "block"
    }
    function relink(msg, duration) {
        duration = isNaN(duration) ? 3000 : duration;
        var m = document.createElement("div");
        m.style.cssText = "position: absolute;width: 100%;text-align: center;bottom: 20%;z-index: 999999;";
        var n = document.createElement("div");
        var width = msg.length * 15;
        n.innerHTML = msg;
        n.style.cssText = "margin: auto;width: " + width + "px;min-width: 150px;opacity: 0.7;height: 50px;color: rgb(255, 255, 255);line-height: 40px;text-align: center;border-radius: 10px;background: rgb(0, 0, 0);font-size: 15px;";
        m.appendChild(n);
    ...
    window.copyRegister = copyRegister;
