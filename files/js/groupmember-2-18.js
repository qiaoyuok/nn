var httpModule = {
        ...
        Vue.http.post(globalData.baseUrl + 'manage/searchGroupMember', data).then(function(response) {
            ...
            if (bodyData.result == 0) {
                ...
                for (var i = 0; i < bodyData.data.length;i++) {
                    ...
                    appData.members.push({"id":item.account_id,"name":item.nickname,"mname":item.mname,"avatarUrl":item.avatar_url,"status":item.status,"pos":item.pos,"manager_id":item.manager_id,"member_id":item.member_id });
                }
	...
	var data = {
        ...
        Vue.http.post(globalData.baseUrl + 'manage/setManager', data)
    },
	...
	dealChangeMname: function(item) {
		var data = {
			"account_id": item.id,
			"mname": item.mname,
			"manager_id": item.manager_id,
		};
        	Vue.http.post(globalData.baseUrl + 'manage/dealChangeMname', data);
	},
...
var viewMethods = {
	...
	hideDelete: function() {
		appData.isShowDelete = false;
	},
	changeMname: function(item) {
		appData.isShowDelete = true;
		appData.nowItem = item;
		appData.nowName = item.name;
		appData.nowMname = item.mname;
	},
	ensureChangeMname: function() {
		appData.nowItem.mname = appData.nowMname;
        	httpModule.dealChangeMname(appData.nowItem);
		appData.isShowDelete = false;
	},
	changeShowSetting: function(show, item) {
		appData.nowItem = item;
		appData.isShowSetting = show;
	},
	setManager: function(bl) {
		appData.isShowSetting = false;
        	httpModule.setManager(appData.nowItem, bl);
		if(bl) {
			appData.nowItem.pos = 1;
		}
		else {
			appData.nowItem.pos = 0;
		}
	},
	kickout: function() {
		appData.nowItem.status = 3;
		var item = appData.nowItem;
        	appData.selectAccount = item;
		appData.isShowSetting = false;
        	httpModule.dealMember(item, 3);
	},
...
var methods = {
	...
	hideDelete: viewMethods.hideDelete,
	changeMname: viewMethods.changeMname,
	ensureChangeMname: viewMethods.ensureChangeMname,
	changeShowSetting: viewMethods.changeShowSetting,
	setManager: viewMethods.setManager,
	kickout: viewMethods.kickout,
