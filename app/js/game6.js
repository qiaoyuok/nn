...
function overroom(data){
    ...
    img.onload = function(){
        ...
        ctx.fillStyle = "#f4db0b";
        ctx.fillText("房间号:7"+sj.id, 211,307);
        ctx.fillText(time1, 372, 307);
        ctx.fillText(sj.zjs+"局", 555, 307);
        ...
        ctx.fillText(writeContent,writeLeft,365);


        for(var i=0;i<sj.user.length;i++){
            if(i>1){
                ctx.fillStyle = "#07463e";
                ctx.fillRect(120,557-(0-(i-2)*84),561,77);
            }
            var user=sj.user[i];
            console.log(user)
            ctx.font = "31px bolder songti";
            ctx.fillStyle = "#d8cb66";
            ctx.fillText(user.nickname.substring(0,9), 235, 436+(i*85));
            if(user.dqjf>0){
                user.dqjf='+'+user.dqjf;
            }else{
                ctx.fillStyle = "#fff";
                ctx.fillText(user.nickname.substring(0,9), 235, 436+(i*85));
                ctx.fillText(user.dqjf,  531, 436+(i*85));
            }
            ctx.fillText(user.dqjf,  531, 436+(i*85));
        }
